# KEXP-20260826-022 — longitudinal live-meta demand response

Status: **COMPLETE / MECHANISM CONFIRMED / POLICY NOT YET PROMOTED**

> Renumbering note: this study was initially written under `KEXP-020`, but the demand-response sequence was renumbered to avoid colliding with the pre-existing `KEXP-019` late-animal study. `KEXP-022` is the canonical identity; the original `KEXP-020` path remains only as a historical alias.

## Prize-first question

KEXP-021 found a striking same-day association in the top-20 official 2026-08-25 ladder episodes: winners bought far more CARROT seed during steps 600–671, especially as public carrot-demand weight from the full eight-shop multiset increased. However, those 20 episodes were only Crop Dusta vs Ryo Hasegawa, so team identity was a serious confounder.

KEXP-022 repeated the exact same measurement over several recent daily ladder datasets before any Kculture policy change.
...
Each episode contributes both players: up to 80 player-games total. Primary variables are identical to KEXP-021:
...
This is enough to reject the hypothesis that the KEXP-021 relationship was merely a Crop Dusta identity artifact. Team identity remains forbidden as a deployment feature.
...
**KEXP-022 supports opening a bounded R4D late-demand-response development candidate.** It does not justify submission or validation access by itself.
...
Build one or two conservative R4D crop-response variants, predeclare their exact triggers, and evaluate pairwise against frozen R4B. Prefer a price/demand-aware trigger over a pure demand threshold because KEXP-022 demonstrates demand-level counterexamples.
