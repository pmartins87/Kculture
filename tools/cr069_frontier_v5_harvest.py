...
TARGETS={
 'v5':('lynnsakurai/farming-score-v5-timing-optimized','R4D_CR069A_FARMING_SCORE_V5_SEATSAFE_V1.tar.gz'),
 'island':('destbreso/island-ga-an-owned-schedule-is-a-moat','R4D_CR069B_ISLAND_GA_SEATSAFE_V1.tar.gz'),
}
...
def build(handle,outname,outdir):
 with tempfile.TemporaryDirectory(prefix='cr069-') as td:
  ...
  # Prefer the conventional submission archive; otherwise require one unique byte stream.
  chosen=None
  for p,n in arcs:
  ...
  return {'handle':handle,'status':'PASS','source_archive':src.name,'source_main_sha256':source_hash,'archive':dst.name,'archive_sha256':sha(dst),'archive_bytes':dst.stat().st_size}
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--output-dir',required=True); a=ap.parse_args(); out=Path(a.output_dir)
 r={'schema_version':'cr069-v1','automatic_kaggle_submission':False,'targets':{}}
