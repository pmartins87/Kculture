#!/usr/bin/env python3
"""V15B fresh paired validation for frozen O-LQ4E.

Dormant harness.  Population, seeds, source hashes and PASS criteria are frozen in the
V15A protocol and configs/all3_v15b_lq4e_fresh_validation.json.
"""
...
def prefire_parity(base,treatment):
    ...
    return (
      [x for x in base["trace"] if int(x[0])<=first]
      ==
      [x for x in treatment["trace"] if int(x[0])<=first]
    )
...
def main():
    ...
    sources=list(cfg.get("sources") or []);seeds=[int(x) for x in cfg.get("seeds") or []];seats=[int(x) for x in cfg.get("seats") or []]
    if len(sources)!=10 or seeds!=[78201,78202,78203,78204] or seats!=[0,1]:
        raise SystemExit("frozen V15B population mismatch")

    rows=[];failures=[];provenance={};started=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="v15b-lq4e-") as td:
        root=Path(td)
        try:
        ...
        if base_main is not None:
            for i,s in enumerate(sources):
                try:
                    ...
                    print("V15B_ACQUIRE",json.dumps({"rank":s["rank"],"ref":s["ref"],"sha":s["main_sha256"]},sort_keys=True),flush=True)
                    time.sleep(1.0)
                except Exception as exc:
                ...
                for seed in seeds:
                    for seat in seats:
                        ...
                        try:
                            ...
                            if any(int(x["turn"])>=336 for x in t["fire_meta"]):raise RuntimeError("late O-LQ4E fire")
                            row={**key,
                            ...
                            rows.append(row);print("V15B_PAIR",json.dumps(row,sort_keys=True),flush=True)
                        except Exception as exc:
    ...
    if not mech:decision="V15B_MECHANICS_INVALID"
    elif not coverage:decision="V15B_LQ4E_UNDERPOWERED"
    elif pass_gate:decision="V15B_LQ4E_FRESH_PASS"
    else:decision="V15B_LQ4E_FRESH_FAIL_CLOSE"
    result={
      "schema":"kculture-all3-v15b-lq4e-fresh-validation-v1","engine":EXPECTED_ENGINE,
