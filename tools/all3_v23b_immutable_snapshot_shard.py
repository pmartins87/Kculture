#!/usr/bin/env python3
"""V23B immutable-snapshot domain upper-bound shard."""
...
def hand_diff_count(a,b):
    aa=list(a or []); bb=list(b or []); n=max(len(aa),len(bb))
    return sum((aa[i] if i<len(aa) else None)!=(bb[i] if i<len(bb) else None) for i in range(n))
...
class HybridCandidate:
    def __init__(self,base_main,teacher_main,mode):
        ...
        self.state=OptionHostState(); self.mode=mode
        self.stats={"turns":0,"market_diff_turns":0,"farmer_diff_turns":0,"hand_diff_total":0,"hybrid_changed_turns":0,"teacher_evaluations":0}
    def __call__(self,obs,config=None):
        ...
        if self.mode=="MARKET_ONLY":
            ...
            out=copy.deepcopy(all3); out["farmer"]=copy.deepcopy(shadow.get("farmer")); out["hands"]=copy.deepcopy(shadow.get("hands") or [])
        elif self.mode=="FULL_SHADOW":
            out=copy.deepcopy(shadow)
        else: raise RuntimeError(f"unknown mode {self.mode}")
...
def run_one(base_main,teacher_main,ctx,mode):
    ...
    margin=mine-other
    return {"score":float(score(margin)),"margin":float(margin),"result":"W" if margin>0 else ("L" if margin<0 else "T"),"rewards":rewards,"steps":steps,"diff_stats":cand.stats}
...
def main():
    ...
    if sha256_bytes(base_main.read_bytes())!=BASE["expected_main_sha256"]:
        raise SystemExit("snapshot base SHA mismatch")

    source_paths={}
    for src in manifest.get("sources",[]):
        p=root/src["path"]; got=sha256_bytes(p.read_bytes())
        if got!=src["sha"]: raise SystemExit(f"snapshot source SHA mismatch {got} != {src['sha']}")
        source_paths[src["sha"]]=p
    ...
    for _idx,ctx in selected:
        ...
        teacher=source_paths.get(sha)
        if teacher is None:
            failures.append({"phase":"snapshot_lookup","context_id":ctx["context_id"],"sha":sha,"error":"source SHA missing from immutable snapshot"})
            continue
        base_result=None
        for mode in MODES:
            try:
                ...
                if mode=="BASE":
                    if float(rr["score"])!=float(ctx["base_score"]) or float(rr["margin"])!=float(ctx["base_margin"]):
                        raise RuntimeError(f"BASE replay mismatch {(rr['score'],rr['margin'])} != {(ctx['base_score'],ctx['base_margin'])}")
                    base_result=rr
                if base_result is None: raise RuntimeError("BASE must run first")
                row={
                  "context_id":ctx["context_id"],"source_rank":ctx.get("rank"),"ref":ctx["ref"],"main_sha256":sha,
                  "seed":int(ctx["seed"]),"seat":int(ctx["seat"]),"mode":mode,
                  "base_score":float(base_result["score"]),"treatment_score":float(rr["score"]),"score_delta":float(rr["score"])-float(base_result["score"]),
                  "base_margin":float(base_result["margin"]),"treatment_margin":float(rr["margin"]),"margin_delta":float(rr["margin"])-float(base_result["margin"]),
                  "diff_stats":rr["diff_stats"],
                }
                rows.append(row)
                print("V23B_MODE",json.dumps({k:row[k] for k in ("context_id","source_rank","seed","seat","mode","score_delta","margin_delta")},sort_keys=True),flush=True)
            except Exception as exc:
                failures.append({"phase":"episode","context_id":ctx["context_id"],"mode":mode,"error":f"{type(exc).__name__}: {exc}"})

    expected=len(selected)*len(MODES)
    mechanical_pass=not failures and len(rows)==expected
    result={
      "schema":"kculture-all3-v23b-domain-upper-bound-shard-v1","engine":EXPECTED_ENGINE,
    ...
    print("V23B_SHARD_RESULT",json.dumps({"shard":args.shard_index,"mechanical_pass":mechanical_pass,"contexts":len(selected),"rows":len(rows),"failures":len(failures)},sort_keys=True),flush=True)
