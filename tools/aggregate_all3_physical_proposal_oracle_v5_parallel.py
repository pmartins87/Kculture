#!/usr/bin/env python3
"""Aggregate execution-equivalent opponent×seed V5 physical oracle shards."""
...
def main():
    ...
    mech=len(shards)==8 and actual==expected and not failures and all(s.get("mechanical_pass") for s in shards) and len(rows)>=12
    ...
    transforms=Counter();sources=Counter()
    for r in rows:
        o=r["oracle"]
        if o.get("label")!="BASE":
    ...
    result={
      "schema":"kculture-all3-physical-proposal-v5-parallel","mechanical_pass":mech,"decision":decision,
