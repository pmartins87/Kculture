#!/usr/bin/env python3
"""V28B current-frontier immutable snapshot discovery."""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="v28b-frontier-") as td:
        tmp=Path(td);by_sha={}
        for rank,ref in enumerate(refs,start=1):
            try:
                ...
                print("V28B_ACQUIRE",json.dumps({"rank":rank,"ref":ref,"sha":sha},sort_keys=True),flush=True)
            except Exception as exc:
                rec={"rank":rank,"ref":ref,"status":"UNAVAILABLE","error":f"{type(exc).__name__}: {exc}"}
                acquisitions.append(rec);unavailable.append(rec)
                print("V28B_UNAVAILABLE",json.dumps(rec,sort_keys=True),flush=True)
        ...
        for sha,meta in sorted(by_sha.items(),key=lambda kv:min(kv[1]["ranks"])):
            ...
            print("V28B_SMOKE",json.dumps({"sha":sha,"rank":rr,"ref":ref,"pass_both":pass_both,"is_v47_identity":row["is_v47_identity"]},sort_keys=True),flush=True)
        ...
        manifest={"schema":"kculture-v28b-current-frontier-snapshot-v1","base":{"ref":BASE["handle"],"sha":BASE["expected_main_sha256"],"path":"base/main.py","receipt":base_receipt},"sources":manifest_sources}
    ...
    decision="V28B_FRONTIER_SNAPSHOT_READY" if mechanical_pass else "V28B_FRONTIER_SNAPSHOT_INVALID"
    result={"schema":"kculture-v28b-frontier-snapshot-result-v1","decision":decision,"mechanical_pass":mechanical_pass,"top30_refs":refs,"acquisitions":acquisitions,"unavailable_refs":unavailable,"unique_sources":len(unique_rows),"selected_representatives":selected,"selected_representative_count":len(selected),"smoke_failures":smoke_failures,"snapshot_manifest":"MANIFEST.json","automatic_kaggle_submission":False}
    outp.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
    print("V28B_SNAPSHOT_RESULT",json.dumps({"decision":decision,"mechanical_pass":mechanical_pass,"selected_representatives":len(selected),"unavailable_refs":len(unavailable),"smoke_failures":len(smoke_failures)},sort_keys=True),flush=True)
