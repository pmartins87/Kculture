...
opening clusters, turn-level consensus, and public farm/economy checkpoints.
...
def actions(rep: dict, p: int) -> list:
    ...
    out = []
    # action chosen from observation at state t is recorded on frame t+1
...
def summarize_checkpoints(cps: dict[int, list[dict]]) -> dict:
    ...
    for t, rows in sorted(cps.items()):
        ...
        out[str(t)] = {
            "n": len(rows),
            "money_mean": statistics.mean(r["money"] for r in rows),
            ...
            "crops_mean": mean_dict(rows, "crops"),
            "animals_mean": mean_dict(rows, "animals"),
...
def main() -> None:
    ...
    ap.add_argument("replay_root")
    ap.add_argument("--team", required=True)
    ap.add_argument("--output", required=True)
    ap.add_argument("--max-turns", type=int, default=719)
    args = ap.parse_args()
    ...
    seqs = []
    cps = {t: [] for t in CHECKPOINTS}
    episodes = []
    errors = []
    for path in files:
        try:
            rep = json.loads(path.read_text(encoding="utf-8"))
            p = target_seat(rep, args.team)
            seq = actions(rep, p)[: args.max_turns]
            seqs.append(seq)
            for t in CHECKPOINTS:
                cps[t].append(checkpoint(rep, p, t))
            final = rep.get("steps", [[]])[-1]
            rewards = []
            for i in (0, 1):
                try:
                    rewards.append(float(final[i].get("reward")))
                except Exception:
                    rewards.append(None)
            episodes.append({
                "file": str(path),
                "seat": p,
                "team_names": team_names(rep),
    ...
    if not seqs:
        raise SystemExit("no usable target-team replays")

    max_turns = min(args.max_turns, max(len(s) for s in seqs))
    turn_rows = []
    modal_actions = []
    for t in range(max_turns):
        vals = [canon(s[t]) for s in seqs if t < len(s)]
        c = collections.Counter(vals)
        modal, count = c.most_common(1)[0]
        modal_actions.append(json.loads(modal))
        turn_rows.append({
            "turn": t,
            "n": len(vals),
            "unique_actions": len(c),
            "modal_fraction": count / len(vals),
            "entropy_bits": entropy(c),
            "modal_action": json.loads(modal),
            "top_actions": [
                {"count": n, "fraction": n / len(vals), "action": json.loads(raw)}
                for raw, n in c.most_common(5)
            ],
        })
    ...
    for width in (2, 8, 16, 32, 64):
        ...
        for seq in seqs:
            raw = "\n".join(canon(x) for x in seq[:width])
            c[hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]] += 1
        opening_clusters[str(width)] = [{"hash": h, "count": n} for h, n in c.most_common()]

    consensus = [r["modal_fraction"] for r in turn_rows]
    payload = {
        "schema_version": "kculture-hosted-policy-corpus-fingerprint-v1",
        "team": args.team,
        "usable_replays": len(seqs),
        "errors": errors,
        "max_turns": max_turns,
        "policy_shape": {
            "unanimous_turns": sum(x == 1.0 for x in consensus),
            "turns_modal_ge_0_90": sum(x >= 0.90 for x in consensus),
            "turns_modal_ge_0_75": sum(x >= 0.75 for x in consensus),
            "mean_modal_fraction": statistics.mean(consensus),
            "median_modal_fraction": statistics.median(consensus),
            "first_nonunanimous_turn": next((i for i, x in enumerate(consensus) if x < 1.0), None),
        },
        "opening_clusters": opening_clusters,
        "checkpoints": summarize_checkpoints(cps),
        "turns": turn_rows,
        "modal_actions": modal_actions,
        "episodes": episodes,
        "note": "Descriptive public-replay evidence only; no causal or promotion claim.",
    }
    ...
    print(json.dumps({
        ...
        "opening_clusters_32_top5": opening_clusters["32"][:5],
