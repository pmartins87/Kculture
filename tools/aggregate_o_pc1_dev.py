#!/usr/bin/env python3
"""Aggregate frozen O-PC1 development gate."""
...
def main():
    ...
    for opp in sorted({r["opponent"] for r in rows}):
        ...
        by[opp]={
          "contexts":len(xs),
          "mean_score_delta":statistics.fmean(float(r["score_delta"]) for r in xs),
          "mean_margin_delta":statistics.fmean(float(r["margin_delta"]) for r in xs),
    ...
    result={
      "schema":"kculture-o-pc1-development-v1","mechanical_pass":mech,
      "decision":decision,"summary":summary,"by_opponent":by,
