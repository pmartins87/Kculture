...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="v33a-") as td:
      ...
      (root/"MANIFEST.json").write_text(json.dumps({"schema":"kculture-v33a-expanded-public-snapshot-v1","sources":srcs},indent=2,sort_keys=True)+chr(10))
    mech=len(refs)>=80
    ...
    Path(a.out).parent.mkdir(parents=True,exist_ok=True);Path(a.out).write_text(json.dumps(out,indent=2,sort_keys=True)+chr(10))
