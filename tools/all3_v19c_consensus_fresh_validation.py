#!/usr/bin/env python3
"""V19C fresh paired validation for frozen O-TM1 consensus schedule."""
...
class Candidate:
    def __init__(self,base_main,treatment,schedule):
        ...
        self.treatment=bool(treatment);self.schedule=schedule;self.turn=0;self.trace=[];self.fire_meta=[]
...
def load_schedule(path):
    ...
    if d.get("decision")!="V19A_CONSENSUS_SCHEDULE_READY":raise RuntimeError("schedule not READY")
    return d["schedule"],hashlib.sha256(raw).hexdigest()

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--config",required=True);ap.add_argument("--schedule",required=True);ap.add_argument("--out",required=True)
    args=ap.parse_args()
    ...
    if len(sources)!=10 or seeds!=[78601,78602,78603,78604] or seats!=[0,1]:raise SystemExit("frozen V19C population mismatch")

    rows=[];failures=[];provenance={};started=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="v19c-tm1-") as td:
        root=Path(td)
        try:
                ...
                for seed in seeds:
                    for seat in seats:
                        ...
                        try:
                            ...
                            rows.append(row);print("V19C_PAIR",json.dumps(row,sort_keys=True),flush=True)
                        except Exception as exc:
    ...
    result={
      "schema":"kculture-all3-v19c-consensus-fresh-v1","engine":EXPECTED_ENGINE,
    ...
    print("V19C_RESULT",json.dumps({
      "decision":decision,"mechanical_pass":mech,"coverage_pass":coverage,"pairs":len(rows),
      "fire_contexts":len(fire),"positive_score_contexts":len(pos),"positive_score_sources":len(pos_sources),
