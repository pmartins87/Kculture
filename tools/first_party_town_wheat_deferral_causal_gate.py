#!/usr/bin/env python3
"""First-party causal gate for O-TW1: hold V47 WHEAT sales across a public town pulse.

At a selected causal branch state, if exact hosted-faithful V47 intends to SELL owned
WHEAT on a turn with mechanically known public town WHEAT consumption, suppress only
those SELL WHEAT orders for that turn. Farmer, hands and every other market order remain
exact V47. Exact V47 resumes on the next turn.

No opponent-private state, identity, rating, replay metadata, hidden seed or future
state is used by the operator.
"""
...
def own_wheat(obs: Any) -> int:
    p = plain(obs)
    private = getv(p, "private", {}) or {}
    shed = getv(private, "shed", {}) or {}
    return max(0, as_int(getv(shed, "WHEAT", 0), 0))


def town_wheat_demand(step: int, obs: Any, config: Any) -> int:
    """Historical H1 public-mechanics pulse detector, frozen unchanged."""
    demand = 0
    shop_interval = max(1, as_int(getv(config or {}, "townShopSellInterval", 4), 4))
    center_interval = max(1, as_int(getv(config or {}, "townCenterSellInterval", 24), 24))
...
def wheat_sell_qty(action: dict) -> int:
    a = canonical_action(action)
    total = 0
    for order in list(a["market"] or []):
        if not isinstance(order, (list, tuple)) or len(order) < 3:
            continue
        if str(order[0]) != "SELL" or str(order[1]) != "WHEAT":
            continue
        total += max(0, as_int(order[2], 0))
    return total


def eligible(obs: Any, config: Any, base_action: dict) -> bool:
    step = obs_step(obs)
    if step < 0 or step > MAX_STEP:
        return False
    demand = town_wheat_demand(step, obs, config)
    qty = wheat_sell_qty(base_action)
    return demand > 0 and qty > 0 and own_wheat(obs) >= qty


def treated_action(base_action: dict) -> tuple[dict, int]:
    ...
    for order in list(a["market"] or []):
        if (
            ...
            removed += max(0, as_int(order[2], 0))
            continue
        market.append(copy.deepcopy(order))
    if removed <= 0:
        raise RuntimeError("O-TW1 requires at least one exact V47 SELL WHEAT order")
    return {
    ...
    def __call__(self, obs, config=None):
        ...
        if eligible(obs, config, base):
            ...
            self.trace.append({
                ...
                "wheat": own_wheat(obs),
                "sell_qty": wheat_sell_qty(base),
                "town_demand": town_wheat_demand(step, obs, config),
            })
        return base
...
class Treatment:
    def __init__(self, main_py: Path, event: dict):
        ...
        self.expected_sell_qty = int(event["sell_qty"])
        self.expected_town_demand = int(event["town_demand"])
    ...
    def __call__(self, obs, config=None):
        ...
        if action_key(base) != self.expected_base_key:
            raise RuntimeError("V47 base action mismatch at O-TW1 target")
        if not eligible(obs, config, base):
            raise RuntimeError(
                f"O-TW1 target no longer eligible step={self.target_step} "
                f"wheat={own_wheat(obs)} sell_qty={wheat_sell_qty(base)} "
                f"town_demand={town_wheat_demand(self.target_step, obs, config)}"
            )
        demand = town_wheat_demand(self.target_step, obs, config)
        if demand != self.expected_town_demand:
            raise RuntimeError(
                f"O-TW1 public pulse mismatch {demand} != {self.expected_town_demand}"
            )

        out, removed = treated_action(base)
        if removed != self.expected_sell_qty:
            raise RuntimeError(
                f"O-TW1 removed qty mismatch {removed} != {self.expected_sell_qty}"
            )
        if out["farmer"] != base["farmer"] or out["hands"] != base["hands"]:
            raise RuntimeError("O-TW1 changed physical action")
        self.removed_qty = removed
        return out
...
def finish(env, seat: int, wrapper=None) -> dict:
    ...
    if wrapper is not None and hasattr(wrapper, "target_seen") and not wrapper.target_seen:
        raise RuntimeError(f"O-TW1 target {wrapper.target_step} not reached")
    ...
    if wrapper is not None and hasattr(wrapper, "removed_qty"):
        out["removed_qty"] = int(wrapper.removed_qty)
    return out
...
def run_episode(base_main: Path, opp_main: Path, *, seed: int, seat: int, mode: str, event=None):
    ...
    if mode == "discovery":
        ...
        out["base_entrypoint"] = cand.entrypoint
    return out
...
def summarize(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "sell_qty_values": sorted({int(r["sell_qty"]) for r in rows}),
        "town_demand_values": sorted({int(r["town_demand"]) for r in rows}),
    }
...
def main() -> None:
    ...
    ap.add_argument(
        "--out",
        default="artifacts/town-wheat-deferral-causal/TOWN_WHEAT_DEFERRAL_CAUSAL_GATE.json",
    )
    args = ap.parse_args()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="town-wheat-deferral-causal-") as td:
            tmp = Path(td)
                ...
                for seed in SEEDS:
                    for seat in (0, 1):
                        ...
                        try:
                            ...
                            if d.get("base_entrypoint") != "_y_agent_shopherd":
                                raise RuntimeError(
                                    f"discovery entrypoint mismatch: {d.get('base_entrypoint')}"
                                )
                            purge(all_paths)
                            b = run_episode(
                            ...
                            if d["rewards"] != b["rewards"]:
                                raise RuntimeError(
                                    f"fresh replay parity mismatch {d['rewards']} != {b['rewards']}"
                                )
                            ...
                            matchups.append({
                                **key,
                                ...
                                "selected_sell_qty": [int(e["sell_qty"]) for e in events],
                                "selected_town_demand": [int(e["town_demand"]) for e in events],
                                "replay_parity": True,
                            })
                            ...
                            for event in events:
                                ...
                                row = {
                                    **key,
                                    "step": int(event["step"]),
                                    "wheat": int(event["wheat"]),
                                    "sell_qty": int(event["sell_qty"]),
                                    "town_demand": int(event["town_demand"]),
                                    "base": {
                                        "reward": d["reward"],
                                    ...
                                    "treatment": {
                                        ...
                                        "margin": tr["margin"],
                                        "score": tr["score"],
                                        "removed_qty": tr.get("removed_qty", 0),
                                    },
                                }
                                rows.append(row)
                                print(
                                    "TOWN_WHEAT_DEFERRAL_BRANCH",
                                    json.dumps(
                                        {
                                            **key,
                                            "step": row["step"],
                                            ...
                                            "town_demand": row["town_demand"],
                                            "base_score": row["base"]["score"],
                                            "treatment_score": row["treatment"]["score"],
                                            "score_delta": row["treatment"]["score"]
                                            - row["base"]["score"],
                                            "margin_delta": row["treatment"]["margin"]
                                            - row["base"]["margin"],
                                        },
                                        sort_keys=True,
                                    ),
    ...
    if mechanical_pass and (
        ...
        and nonnegative_blocks >= 2
    ):
        decision = "TOWN_WHEAT_DEFERRAL_CAUSAL_PASS_SAFE_OPTION"
    elif mechanical_pass and (
        int(overall.get("positive_wl_states", 0)) > 0
        and (
            int(overall.get("negative_wl_states", 0)) > 0
            or any(
                v and float(v.get("mean_score_delta", 0.0)) < 0
                for v in by_opp.values()
            )
        )
    ):
        decision = "TOWN_WHEAT_DEFERRAL_CAUSAL_HETEROGENEOUS"
    elif mechanical_pass and float(overall.get("mean_margin_delta", 0.0)) > 0:
        decision = "TOWN_WHEAT_DEFERRAL_MARGIN_ONLY"
    elif mechanical_pass:
        decision = "TOWN_WHEAT_DEFERRAL_NO_HEADROOM"
    else:
        decision = "TOWN_WHEAT_DEFERRAL_MECHANICS_INVALID"

    result = {
        "schema": "kculture-first-party-town-wheat-deferral-causal-v1",
        ...
        "operator": {
            "name": "O-TW1",
            "eligibility": (
                "public town WHEAT demand this turn and exact V47 currently SELLs "
                "owned WHEAT"
            ),
            "treatment": (
                "remove only current-turn SELL WHEAT orders; exact V47 resumes next turn"
            ),
            "max_step": MAX_STEP,
            "max_events_per_matchup": MAX_EVENTS_PER_MATCHUP,
            "min_event_separation": MIN_EVENT_SEPARATION,
            "minimum_valid_branch_states": MIN_VALID_BRANCH_STATES,
        },
