"""Apply the frozen CR080 confirmation gate; no parameter/strategy selection."""
...
def main():
    ...
    required=[('CR080','CR071M')]+[(v,o) for o in guards for v in ['CR080','CR071M']]
    missing=[x for x in required if x not in games]
    report={'missing':missing,'primary_metric':'seat-balanced W/L','automatic_kaggle_submission':False}
    checks={'complete_panel':not missing}
    if not missing:
        checks['all_64_games_32_fresh_seeds']=all(games[x]['metrics_a_vs_b']['games']==64 and games[x]['seed_count']==32 and games[x]['master_seed']==9112081 for x in required)
        ...
        delta={o:rate('CR080',o)-rate('CR071M',o) for o in guards}
        checks['direct_at_least_0_5625']=rate('CR080','CR071M')>=.5625
        ...
        report.update({'guardrail_deltas':delta,'rows':[{'a':x[0],'b':x[1],**games[x]['metrics_a_vs_b']} for x in required]})
    report['checks']=checks;report['pass']=all(checks.values());report['decision']='ELIGIBLE_FOR_ONE_HOSTED_PROBE_AFTER_SLOT_ACCOUNTING' if report['pass'] else 'CLOSE_CR080_NO_RETUNING'
