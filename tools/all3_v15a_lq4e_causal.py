#!/usr/bin/env python3
"""V15A paired causal discovery for frozen O-LQ4E on all 24 V13C hard contexts."""
...
from tools.first_party_lq4e_early_fertilizer_before_hire import lq4e_action
...
def run_episode(base_main,opp_main,ctx,treatment):
    ...
    if st!=["DONE","DONE"] or len(rw)!=2 or steps<720 or not all(math.isfinite(x) for x in rw):
        raise RuntimeError(f"invalid episode statuses={st} rewards={rw} steps={steps}")
    mine,other=(rw[0],rw[1]) if int(ctx["seat"])==0 else (rw[1],rw[0]);m=mine-other
    return {
      "score":score(m),"margin":m,"result":"W" if m>0 else ("L" if m<0 else "T"),
      "rewards":rw,"trace":cand.trace,"fire_meta":cand.fire_meta,
      "fire_count":len(cand.fire_meta),"seconds":secs,
    }

def prefire_parity(base,treatment):
    fires=treatment.get("fire_meta") or []
    if not fires:
        return base["trace"]==treatment["trace"] and base["rewards"]==treatment["rewards"]
    first=min(int(x["turn"]) for x in fires)
    bt=[x for x in base["trace"] if int(x[0])<=first]
    tt=[x for x in treatment["trace"] if int(x[0])<=first]
    return bt==tt
...
def main():
    ...
    if len(contexts)!=24:raise SystemExit(f"expected 24 contexts, got {len(contexts)}")

    failures=[];rows=[];provenance={};started=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="v15a-lq4e-") as td:
        root=Path(td)
        try:
        ...
        if base_main is not None:
            ...
            for i,(sha,ref) in enumerate(source_ref.items()):
                try:
                    ...
                    print("V15A_ACQUIRE",json.dumps({"ref":ref,"sha":sha},sort_keys=True),flush=True)
                    time.sleep(1.0)
                except Exception as exc:
        ...
        if base_main is not None:
            for c in contexts:
                ...
                try:
                    ...
                    if not prefire_parity(b,t):
                        raise RuntimeError("pre-fire parity failure")
                    if any(int(x["turn"])>=336 for x in t["fire_meta"]):
                        raise RuntimeError("late O-LQ4E fire")
                    row={
                    ...
                    rows.append(row)
                    print("V15A_PAIR",json.dumps(row,sort_keys=True),flush=True)
                except Exception as exc:
    ...
    fire_rows=[r for r in rows if int(r["fire_count"])>0]
    fire_sources={r["main_sha256"] for r in fire_rows}
    ltw=[r for r in rows if float(r["base_score"])==0.0 and float(r["treatment_score"])==1.0]
    ltw_sources={r["main_sha256"] for r in ltw}
    coverage=(mech and len(fire_rows)>=8 and len(fire_sources)>=2)
    ...
    if not mech:decision="V15A_MECHANICS_INVALID"
    elif not coverage:decision="V15A_LQ4E_UNDERPOWERED"
    elif len(ltw)>=4 and len(ltw_sources)>=2 and mean_score>0 and mean_margin>0:
        decision="V15A_LQ4E_WL_HEADROOM"
    elif mean_margin>0:
        decision="V15A_LQ4E_MARGIN_ONLY_CLOSE"
    else:
        decision="V15A_LQ4E_NO_HEADROOM_CLOSE"

    result={
      "schema":"kculture-all3-v15a-lq4e-causal-v1","engine":EXPECTED_ENGINE,
      "mechanical_pass":mech,"coverage_pass":coverage,"decision":decision,
      "contexts":len(rows),"fire_contexts":len(fire_rows),"fire_sources":len(fire_sources),
    ...
    print("V15A_RESULT",json.dumps({
      "decision":decision,"mechanical_pass":mech,"coverage_pass":coverage,
      "fire_contexts":len(fire_rows),"fire_sources":len(fire_sources),"total_fires":result["total_fires"],
      "loss_to_win_flips":len(ltw),"loss_to_win_sources":len(ltw_sources),
