#!/usr/bin/env python3
"""V17C fresh paired validation for frozen O-LQ6S rising-edge STRAWBERRY option."""
...
class Candidate:
    ...
    def __call__(self,obs,config=None):
        ...
        out,meta=lq6s_action(obs,all3,t,self.trigger_name,self.edge)
        if meta.get("fired"):self.fire_meta.append(meta)
        return out

def run_episode(base_main,opp_main,seed,seat,treatment,trigger_name):
...
def main():
    ...
    ap.add_argument("--config",required=True);ap.add_argument("--trigger-config",required=True);ap.add_argument("--out",required=True)
    args=ap.parse_args()
    import kaggle_environments
    if str(getattr(kaggle_environments,"__version__",""))!=EXPECTED_ENGINE:raise SystemExit("engine mismatch")
    cfg=json.loads(Path(args.config).read_text());sources=list(cfg.get("sources") or [])
    ...
    if len(sources)!=10 or seeds!=[78401,78402,78403,78404] or seats!=[0,1]:raise SystemExit("frozen V17C population mismatch")
    ...
    with tempfile.TemporaryDirectory(prefix="v17c-lq6s-") as td:
        root=Path(td)
        try:
            ...
            for s in sources:
                ...
                for seed in seeds:
                    for seat in seats:
                        ...
                        try:
                            b=run_episode(base_main,opp,seed,seat,False,trigger_name)
                            t=run_episode(base_main,opp,seed,seat,True,trigger_name)
                            ...
                            rows.append(row);print("V17C_PAIR",json.dumps(row,sort_keys=True),flush=True)
                        except Exception as exc:
    ...
    if not mech:decision="V17C_MECHANICS_INVALID"
    elif not coverage:decision="V17C_LQ6S_UNDERPOWERED"
    elif pass_gate:decision="V17C_LQ6S_FRESH_PASS"
    else:decision="V17C_LQ6S_FRESH_FAIL_CLOSE"
    result={
      "schema":"kculture-all3-v17c-lq6s-fresh-validation-v1","engine":EXPECTED_ENGINE,
