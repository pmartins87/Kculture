#!/usr/bin/env python3
"""Fresh full-episode autonomous runtime gate for frozen first-party O-TW1.

Treatment = exact hosted-faithful V47 plus one-shot:
  at the first state where public town mechanics imply WHEAT consumption this turn
  and exact V47 wants to SELL already-owned WHEAT, suppress only those SELL WHEAT
  orders for that turn; then disable O-TW1 for the rest of the episode.

No forced later sale, no opponent-private state, no identity/rating/seed feature.
"""
...
class OneShotTownWheatHold:
    def __init__(self, main_py: Path):
        self.agent = load_public_agent(main_py)
        self.trace: list[tuple[str, str]] = []
        self.used = False
        self.trigger_step: int | None = None
        self.trigger_wheat: int | None = None
        self.trigger_sell_qty: int | None = None
        self.trigger_town_demand: int | None = None

    def __call__(self, obs, config=None):
        base = canonical_action(call_agent(self.agent, obs, config))
        self.trace.append((obs_hash(obs), action_key(base)))
        if not self.used and eligible(obs, config, base):
            self.used = True
            p = plain(obs)
            try:
                self.trigger_step = int(p.get("step", len(self.trace) - 1))
            except Exception:
                self.trigger_step = len(self.trace) - 1
            self.trigger_wheat = own_wheat(obs)
            self.trigger_sell_qty = wheat_sell_qty(base)
            self.trigger_town_demand = town_wheat_demand(self.trigger_step, obs, config)
            out, removed = treated_action(base)
            if removed != self.trigger_sell_qty:
                raise RuntimeError(
                    f"O-TW1 removed qty mismatch {removed} != {self.trigger_sell_qty}"
                )
...
def run_episode(base_main: Path, opp_main: Path, *, seed: int, seat: int, treatment: bool):
    cand = OneShotTownWheatHold(base_main) if treatment else BaseTrace(base_main)
    ...
    out["trigger_wheat"] = getattr(cand, "trigger_wheat", None)
    out["trigger_sell_qty"] = getattr(cand, "trigger_sell_qty", None)
    out["trigger_town_demand"] = getattr(cand, "trigger_town_demand", None)
    return out
...
def summarize(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "sell_qty_values": sorted({int(r["trigger_sell_qty"]) for r in triggers}),
        "town_demand_values": sorted({int(r["trigger_town_demand"]) for r in triggers}),
...
def main() -> None:
    ...
    ap.add_argument(
        "--out",
        default="artifacts/town-wheat-runtime/TOWN_WHEAT_ONESHOT_RUNTIME_GATE.json",
    )
    args = ap.parse_args()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="town-wheat-runtime-") as td:
            tmp = Path(td)
            ...
            if hosted_entrypoint != "_y_agent_shopherd":
                raise RuntimeError(
                    f"unexpected exact V47 hosted entrypoint: {hosted_entrypoint}"
                )
                        ...
                        try:
                            ...
                            row = {
                                **key,
                                ...
                                "trigger_wheat":tr["trigger_wheat"],
                                "trigger_sell_qty":tr["trigger_sell_qty"],
                                "trigger_town_demand":tr["trigger_town_demand"],
                                "parity":parity,
                            }
                            rows.append(row)
                            print("TOWN_WHEAT_RUNTIME_PAIR",json.dumps({
                                **key,
                                "triggered":row["triggered"],
                                "trigger_step":row["trigger_step"],
                                "sell_qty":row["trigger_sell_qty"],
                                "town_demand":row["trigger_town_demand"],
    ...
    if mechanical_pass and (
        ...
        decision="TOWN_WHEAT_RUNTIME_PASS"
        ...
        decision="TOWN_WHEAT_RUNTIME_HETEROGENEOUS"
    elif mechanical_pass and (
        abs(float(overall.get("score_delta",0.0)))<1e-15
        and float(overall.get("mean_margin_delta",0.0))>0.0
        and worst_block>=-0.0625
    ):
        decision="TOWN_WHEAT_RUNTIME_MARGIN_ONLY"
    elif mechanical_pass:
        decision="TOWN_WHEAT_RUNTIME_FAIL"
    else:
        decision="TOWN_WHEAT_RUNTIME_MECHANICS_INVALID"

    result={
        "schema":"kculture-town-wheat-oneshot-runtime-v1",
        ...
        "operator":{
            "id":"O-TW1",
            "eligibility":"first public town-WHEAT pulse state where V47 sells already-owned WHEAT",
            "treatment":"suppress only current-turn SELL WHEAT orders; exact V47 resumes next turn",
    ...
    print("TOWN_WHEAT_RUNTIME_RESULT",json.dumps({
        "pairs":len(rows),
        "mechanical_pass":mechanical_pass,
        "summary":overall,
        "by_opponent":by_opp,
