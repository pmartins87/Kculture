#!/usr/bin/env python3
"""V8D-E2 final fresh extension shard. O-LQ3C is unchanged."""
...
from tools.o_lq3c_v8d_validation_shard import (
    EXPECTED_ENGINE,BASE,V2_OPPONENTS,acquire,run,parity,purge
)

SEED_BLOCKS={
 "G":[76025,76026,76027,76028],
 "H":[76029,76030,76031,76032],
 "I":[76033,76034,76035,76036],
}
...
def main():
    ...
    try:
      with tempfile.TemporaryDirectory(prefix=f"v8d-e2-{args.opponent}-{args.block}-") as td:
        tmp=Path(td)
        ...
        for seed in SEED_BLOCKS[args.block]:
          for seat in (0,1):
            ...
            try:
                ...
                rows.append(row)
                print("V8D_E2_PAIR",json.dumps(row,sort_keys=True),flush=True)
    ...
    result={"schema":"kculture-v8d-e2-lq3c-shard-v1","mechanical_pass":mech,
    ...
    print("V8D_E2_SHARD_RESULT",json.dumps({"opponent":args.opponent,"block":args.block,"mechanical_pass":mech,
